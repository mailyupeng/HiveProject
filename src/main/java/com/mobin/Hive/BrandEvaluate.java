...
public class BrandEvaluate {
    ...
    public static void query(int lastMth,int thisMth,String lastTime,String thisTime, int type,int Num) throws SQLException {
        ...
        String hql ="";
        if(Num == 0){
            hql = "WITH wtmp AS (SELECT rowkey,to_date(s6) AS time FROM raws WHERE (s3a=='1' OR s9!='1') AND  (mth>=? AND mth<=?) AND to_date(s6)>=?AND to_date(s6)<=?), "+
            "each_g1 AS  "+
            ...
            hql = "WITH wtmp AS (SELECT rowkey,to_date(s6) AS time,cx FROM raws WHERE (s3a=='1' OR s9!='1') AND  (mth>=195 AND mth<=196) AND to_date(s6)>=? AND to_date(s6)<=?), "+
            "each_g1 AS   "+
                    "(SELECT concat_ws(',',model,wtmp.time) AS mt,count(DISTINCT g1) AS g1_cot FROM "+
                                    "(SELECT sid,g1,cbrdint3 AS model FROM bca "+
