        ...
        if(Num == 0){   //日线
            ...
            "(cx_per/brdint3_cot) AS pre FROM "+
                    ...
                    "(SELECT concat_ws(',',model,time) AS mt,count(1) AS cot,model,time FROM "+
                                    "(SELECT sid,cbrdint3 AS model FROM bca "+
                                            "LATERAL VIEW explode(brdint3) tbrdint3 AS cbrdint3 WHERE (s3a==',' OR s9!=',') AND (mth>=? AND mth<=?) AND cbrdint3!='') t3 "+
                            "JOIN wtmp "+
                            "ON base64(t3.sid)=base64(substr(wtmp.rowkey,0,16)) "+
            ...
            "SELECT model,time,brd3_cot,c/brd3_cot AS per,s3a FROM "+
